  ...
  describe('EmbeddingUtils', () => {
    describe('validateEmbedding', () => {
      ...
      it('should reject embedding with wrong dimensions', () => {
        const invalidEmbedding = Array.from({ length: 512 }, () => Math.random());
        expect(EmbeddingUtils.validateEmbedding(invalidEmbedding)).toBe(false);
      });

      it('should reject non-array input', () => {
        expect(EmbeddingUtils.validateEmbedding('not an array' as any)).toBe(false);
      });

      it('should reject embedding with NaN values', () => {
        const invalidEmbedding = Array.from({ length: 1536 }, (_, i) => i === 0 ? NaN : Math.random());
        expect(EmbeddingUtils.validateEmbedding(invalidEmbedding)).toBe(false);
      });
    });
  ...
  describe('VectorSearchService', () => {
    ...
    describe('updateDocumentEmbedding', () => {
      it('should update document embedding successfully', async () => {
        const doc = await KnowledgeDocument.findOne({ title: 'Investment Basics' });
      ...
      it('should reject embedding with wrong dimensions', async () => {
        ...
        await expect(
          VectorSearchService.updateDocumentEmbedding(doc!._id.toString(), invalidEmbedding)
    ...
    describe('batchUpdateEmbeddings', () => {
      it('should update multiple embeddings successfully', async () => {
        ...
        await expect(VectorSearchService.batchUpdateEmbeddings(updates)).resolves.not.toThrow();
    ...
    describe('removeEmbeddings', () => {
      ...
      it('should remove embeddings with filter', async () => {
        ...
        const investmentDoc = await KnowledgeDocument.findOne({ category: 'investment' });
        const budgetingDoc = await KnowledgeDocument.findOne({ category: 'budgeting' });
