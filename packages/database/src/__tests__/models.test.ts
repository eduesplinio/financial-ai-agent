...
describe('Database Models', () => {
  ...
  beforeEach(async () => {
    ...
    try {
      ...
      console.log(
        'Aviso: Não foi possível limpar o banco de dados entre os testes',
        error
      );
    }
  });
  ...
  describe('User Model', () => {
    ...
    const generateUniqueEmail = (testName: string) =>
      `test-${testName}-${Date.now()}@example.com`;

    let validUserData: any; // Permitir tipagem flexível para os testes

    // Gerar dados de usuário com email único para cada teste
    beforeEach(() => {
      validUserData = {
    ...
    describe('Knowledge Document Model', () => {
      ...
      it('should validate document data with Zod schema', () => {
        const result =
          KnowledgeDocumentCreateSchema.safeParse(validDocumentData);
        expect(result.success).toBe(true);
      });
