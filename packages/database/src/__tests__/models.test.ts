...
describe('Database Models', () => {
  ...
  describe('User Model', () => {
    ...
    it('should create a user successfully', async () => {
      ...
      expect(user.profile.riskTolerance).toBe(validUserData.profile.riskTolerance);
  ...
  describe('Transaction Model', () => {
    ...
    it('should find transactions by user ID', async () => {
      ...
      await TransactionService.create({ ...data, amount: -30.00, description: 'Coffee' });
    ...
    it('should find transactions by date range', async () => {
      ...
      const transactions = await TransactionService.findByDateRange(userId, yesterday, today);
    ...
    it('should get spending by category', async () => {
      const data = { ...validTransactionData, userId };
      await TransactionService.create({ ...data, amount: -100, category: { primary: 'food', confidence: 0.9 } });
      await TransactionService.create({ ...data, amount: -50, category: { primary: 'food', confidence: 0.9 } });
      await TransactionService.create({ ...data, amount: -200, category: { primary: 'transport', confidence: 0.9 } });
      
      const startDate = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
      const endDate = new Date();
      
      const spending = await TransactionService.getSpendingByCategory(userId, startDate, endDate);
  ...
  describe('Knowledge Document Model', () => {
    ...
    it('should find documents by category', async () => {
      ...
      const result = await KnowledgeDocumentService.findByCategory('investment');
  ...
  describe('Conversation Model', () => {
    ...
    it('should find conversation by session ID', async () => {
      ...
      const conversation = await ConversationService.findBySessionId('session-123');
    ...
    it('should add message to conversation', async () => {
      ...
      const updatedConversation = await ConversationService.addMessage('session-123', newMessage);
      
      expect(updatedConversation?.messages).toHaveLength(2);
      expect(updatedConversation?.messages[1].content).toBe('Thank you for the advice!');
    });
    ...
    it('should update conversation context', async () => {
      ...
      const updatedConversation = await ConversationService.updateContext('session-123', newContext);
    ...
    it('should find conversations by user ID', async () => {
      const data1 = { ...validConversationData, userId, sessionId: 'session-1' };
      const data2 = { ...validConversationData, userId, sessionId: 'session-2' };
  ...
  describe('Soft Delete Functionality', () => {
    it('should exclude soft deleted documents from queries', async () => {
      ...
      const deletedUser = await User.findOne({ _id: user._id, deletedAt: { $exists: true } });
