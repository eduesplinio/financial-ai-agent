...
export class RateLimiter {
  ...
  constructor(private config: RateLimitConfig = {}) {
    const { tokensPerInterval = 10, interval = 1000, maxTokens = 10 } = config;
    ...
    if (tokensPerInterval > 0) {
      ...
      // Importante: evita que o processo fique preso por causa deste timer
  ...
  /**
   * Limpa recursos utilizados pelo RateLimiter
   * Importante chamar este método quando o RateLimiter não for mais necessário
   */
  public dispose(): void {
    if (this.intervalId) {
    ...
    // Limpa a fila e resolve qualquer promessa pendente
