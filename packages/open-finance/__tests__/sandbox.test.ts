...
describe('OpenFinanceSandbox', () => {
  ...
  test('generateMockAccounts deve criar contas de teste', () => {
    const accounts = sandbox.generateMockAccounts(
      '12345678900',
      'mock-bank-001',
      2
    );
    ...
    expect(accounts[0]).toHaveProperty(
      'institution.name',
      'Banco Sandbox Mock'
    );
  });
  ...
  test('createMockConsent deve criar consentimento de teste', async () => {
    const consent = await sandbox.createMockConsent(
      '12345678900',
      'mock-bank-001'
    );
  ...
  test('enableDebugLogging deve configurar interceptors quando logRequests=true', () => {
    ...
    const sandboxWithLogs = createSandboxClient({
      mockResponses: true,
      logRequests: true,
    });
