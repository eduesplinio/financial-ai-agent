...
describe('OpenFinanceAuth', () => {
  ...
  test('createAuthorizationUrl deve gerar uma URL correta', async () => {
    ...
    expect(result.url).toContain('redirect_uri=https%3A%2F%2Fapp.example.com%2Fcallback');
  });
  ...
  test('exchangeCodeForToken deve fazer requisição correta', async () => {
    ...
    const result = await auth.exchangeCodeForToken(mockOAuthConfig, code, codeVerifier);
  ...
  test('createClientAssertion deve gerar JWT corretamente', async () => {
    ...
    const result = await auth.createClientAssertion(clientId, privateKey, audience);
