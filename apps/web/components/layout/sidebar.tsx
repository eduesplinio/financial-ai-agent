...
export function Sidebar({ className }: SidebarProps) {
  ...
  const SidebarContent = ({ mobile = false }: { mobile?: boolean }) => (
    <>
      ...
      <div
        className={cn(
          'flex-1 py-6 px-3 space-y-1 overflow-y-auto overflow-x-hidden transition-[padding] duration-300 ease-out',
          mobile && 'mt-10'
        )}
      >
        {navItems.map(item => (
          <Tooltip key={item.href}>
            <TooltipTrigger asChild>
              <Link
                ...
                className={cn(
                  'group flex py-3 rounded-lg transition-all duration-200 ease-in-out no-underline',
                  pathname === item.href
              ...
              >
                ...
                >
                  <span className="whitespace-nowrap block transform transition-transform duration-300 ease-out">
                    {item.name}
                  </span>
                </div>
              </Link>
            ...
            {!expanded && !mobile && (
              <TooltipContent
                side="right"
                className="bg-popover/100 border border-border shadow-md backdrop-blur-none"
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <button
                ...
                className="py-3 min-w-[48px] flex items-center justify-center rounded-lg text-muted-foreground hover:bg-muted hover:text-foreground transition-colors duration-200"
      ...
      <div
        className={cn(
          'border-t p-3 transition-all duration-300 ease-out',
          expanded || mobile
        ...
        <DropdownMenu>
          <Tooltip>
            <TooltipTrigger asChild>
              <DropdownMenuTrigger asChild>
                ...
                >
                  ...
                  <div
                    className={cn(
                      'text-left overflow-hidden transition-all duration-300 ease-out',
                      expanded || mobile
                        ? 'w-auto opacity-100 max-w-[200px] translate-x-0'
                        : 'w-0 opacity-0 max-w-0 -translate-x-4'
                    )}
            ...
            <TooltipContent
              ...
              style={{ backgroundColor: 'hsl(var(--popover))' }}
            >
              {expanded ? 'Configurações de perfil' : 'Perfil e configurações'}
            </TooltipContent>
          </Tooltip>
          <DropdownMenuContent
  ...
  return (
    ...
    >
      <div
        className={cn(
          'hidden lg:flex flex-col h-screen bg-background border-r transition-[width] duration-300 ease-out overflow-x-hidden',
          expanded ? 'w-64' : 'w-20',
          className
        )}
      >
